...
module Mscrawler
  class MsacademicSearchResults
    ...
    def to_h
      if @status == 'OK'
        return { 'status' => @status, 'data' => { 'query' => @data['query'], 'start' => @data['start'], 'end' => @data['end'], 'num' => @data['search_results'].length, 'search_results' => @data['search_results'].map { |item| item.to_h } } }
      else
