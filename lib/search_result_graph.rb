...
class SearchResultGraph < CacheableGraph
  ...
  def exists_normal_edge?(source, destination)
    return (exists_edge?(source, destination) and NormalDirectedGraphEdge === @graph[:edges][source.to_s][destination.to_s])
  end

  def exists_between_search_results_edge(source, destination)
    return (exists_edge?(source, destination) and BetweenSearchResultsDirectedGraphEdge === @graph[:edges][source.to_s][destination.to_s])
  end

  def append_edge(edge)
    # NormalDirectedGraphEdgeにBetweenSearchResultsDirectedGraphEdgeを上書きすることは可能だが，逆はできないようにする
    if exists_node?(edge.source.to_s) and exists_node?(edge.destination.to_s)     
      case edge
      when BetweenSearchResultsDirectedGraphEdge
        unless @graph[:edges].key?(edge.source.to_s)
          @graph[:edges][edge.source.to_s] = {}
        end
        @graph[:edges][edge.source.to_s][edge.destination.to_s] = edge
      when NormalDirectedGraphEdge
        unless exists_edge?(edge.source, edge.destination)
          unless @graph[:edges].key?(edge.source.to_s)
            @graph[:edges][edge.source.to_s] = {}
          end
          @graph[:edges][edge.source.to_s][edge.destination.to_s] = edge
